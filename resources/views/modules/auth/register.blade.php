...
<x-views.auth.auth-component :componentData='["left"=>"sideContent","sideHeading" => trans("messages.login.Already_have_an_account"), "sideSubHeading" => trans("messages.login.Already_a_part_of_our_InmoConnect_community"),"sideBtnText" => trans("messages.guest.sign_in"),"mainBtnText"=>trans("messages.guest.create_account"),"formId"=>"registerForm","isInvitedUser" => !empty($invitedUserDetails) ? true : false, "invitedUserDetails" =>  !empty($invitedUserDetails) ? $invitedUserDetails : []]'>
@php
    $mainFormFields = [
            view('components.forms.crm-text-field', [
                'values' => [
                    ...
                    'isRequired' => true,
                    'value' => !empty($invitedUserDetails) ? $invitedUserDetails->name : ''
                ],
            ])->render(),
            ...
            view('components.forms.crm-text-field', [
                'values' => [
                    ...
                    'id' => 'name',
                    'attributes' => [!empty($invitedUserDetails) ? 'readonly' : ''],
                    'hasHelpText' => false,
                    'help' => 'Please enter your email',
                    'isRequired' => true,
                    'value' => !empty($invitedUserDetails) ? $invitedUserDetails->email : ''
                ],
            ])->render(),
        ...
        $mainFormBtn = view('components.forms.crm-button', [
                'values' => [
                    ...
                    'id' => 'createAccountBtn',
                    'attributes' => [ !empty($invitedUserDetails) ? 'data-val = '.getuserRoleName($invitedUserDetails->user_role_id) : ''],
        ...
        $sideFormBtn = view('components.forms.crm-button', [
            'values' => [
                ...
                'target' => '_self',
                'type' => 'button',
                'class' => 'login-white-button color-b-blue b-white-grey font-weight-700 lineheight-18 border-r-8 mt-1 mt-sm-4 border-white text-14',
                'id' => 'signInBtn',
                'attributes' => [],
    ...
    <div class="text-14 color-b-blue font-weight-400 pt-2 lineheight-18 mt-1">{{trans('messages.register.By_signing_up_I_agree_to_the_InmoConnect')}} <a href="#" class="text-decoration-underline color-b-blue"> {{trans('messages.register.Privacy_Policy')}} </a> {{trans('messages.register.and')}} <a href="#" class="text-decoration-underline color-b-blue">{{trans('messages.register.Terms_of_Service')}}</a></div>
    </x-slot>
    ...
    <x-slot name="mainAdditionalContent">
       ...
       <button class="border-r-8 googleSignInBtn socialSignIn" type="button" data-url="{{route(routeNamePrefix().'user.login.google').(!empty($invitedUserDetails) ? '?type='.getuserRoleName($invitedUserDetails->user_role_id) : '')}}" >
        ...
        <button class="border-r-8 twitterSignInBtn socialSignIn" data-url="{{route(routeNamePrefix().'user.login.twitter').(!empty($invitedUserDetails) ? '?type='.getuserRoleName($invitedUserDetails->user_role_id) : '')}}" type="button" >
...
@push('scripts')
<script>
    var routeUrl = "{{ route('user.postRegister').(!empty($invitedUserDetails) ? '?invite-token='.$invitedUserDetails->verification_token : '') }}";
    var asAgentHeading = "{{ trans('messages.create_account.as_agent_heading') }}";2
    var asDeveloperHeading = "{{ trans('messages.create_account.as_developer_heading') }}";
</script> 
