<div class="modal fade" id="uploadProjectDocumentModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-width-change_two" role="document">
        <div class="modal-content modal-content-file">
            <div class="modal-header border-0 modal-header_file pb-0">
                <a class ="projectgobackbtn d-none" data-project-id="{{ $project->id ?? '' }}">
                    < Go Back </a>
                        ...
                        <button type="button" id="projectFileFolderModalClose" class="close b-color-transparent"
            ...
            <div class="modal-body modal-header_file">
                <div class="modal_body_text uploadDocumentModalBody">
                    <form action="" method="post" enctype="multipart/form-data"
                        id="uploadProjectDocumentModalForm">
                        <div class="row project-folders-row">
                ...
                <div class="pt-30 modal-body_footer d-flex justify-content-between align-items-center">
                    <form method="post" enctype="multipart/form-data" class="form_file d-none">
                        ...
                        <label for="apply"> <input type="file" name="file-input" id="apply"
                                accept="image/*,.pdf" class="handleUploadProjectDocumentsFile" multiple>
                            {{ trans('messages.uploads.upload_from_Browser') }}
                        </label>
                    </form>
                    <button type="button"
                        class="create_new_folder_btn delete_project small-button border-r-8  text-14 font-weight-400 lineheight-18 "
                        onclick="openProjectFolderModal()">Create New Folder</button>
                    <button type="button" style="background-color: #f0f0f0"
                        class="small-button border-r-8 text-14 font-weight-400 lineheight-18 color-white {{ !empty($uploadProjectDcoumentsModal) ? $uploadProjectDcoumentsModal : __('') }}"
                        disabled>
                        {{ !empty($submitBtnText) ? $submitBtnText : __('messages.uploads.import') }}</button>
...
<div class="modal fade" id="create_new_project_folder" tabindex="-1"
    style="display: none;background: #00000040; z-index:9999;" role="dialog" aria-labelledby="create_new_folderLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-width-change_eight" role="document">
        <div class="modal-content modal-content-file">
            ...
            <div class="modal-body modal-header_file">
                <div class="row">
                    <form action="" method="post" id="addProjectFolder" enctype="multipart/form-data">
                        ...
                        <div
                            ...
                            <div class="form-group position-relative">
                                <button type="button"
                                    class="Gradient_button small-button border-r-8 b-color-Gradient text-14 font-weight-400 lineheight-18 color-white addProjectFolderBtn">Create
                                    New Folder</button>
                            </div>
                        </div>
                    </form>
...
{{-- view files modal --}}
<div class="modal fade" id="showUploadedProjectDocumentModal" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-width-change_two" role="document">
        <div class="modal-content modal-content-file">
            <div class="modal-header border-0 modal-header_file pb-0">
                <a class ="gobackbtnshowUploadedModal d-none">
                    < Go Back </a>
                        <h4 class="modal-title text-center text-18 font-weight-700 lineheight-22 color-primary w-100"
                            id="dataFilterModalLabel" style="width: 60%">Uploaded Files</h4>
                        <button type="button" id="viewFileFolderModalClose" class="close b-color-transparent"
                            data-bs-dismiss="modal" aria-label="Close">
            ...
            <div class="modal-body modal-header_file">
                <div class="modal_body_text uploadDocumentModalBody">
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <div class="d-flex flex-column">
                                                ...
                                                <a href="javascript:void(0)"
                                                    class="color-primary text-10 text-decoration-underline pt-1 viewfilefolders"
                                                    data-folder-id="{{ $atachment->folder_id }}">{{ $atachment->folderName }}</a>
                                            </div>

                                        </div>
                                        <button type="button" class="b-color-transparent file_remove_btn"
                                            id="" data-id="{{ $atachment->file_id }}"><i
                    ...
                    <form action="" method="post" enctype="multipart/form-data" id="uploadDocumentModalForm">
                        <div class="row view-folders-row">
                            <span id="folder_id"></span>
                            @include('modules.dashboard.includes.project-load-folder', [
                ...
                <div class="pt-30 modal-body_footer d-flex justify-content-end align-items-center">
                    <button type="button"
                        class="Gradient_button small-button border-r-8 text-14 font-weight-400 lineheight-18 color-white data_file_id">Save</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function openProjectFolderModal() {
        let myModal = new bootstrap.Modal(document.getElementById('create_new_project_folder'), {});
        myModal.show();
    }

    $(document).on('click', '.addProjectFolderBtn', function(e) {
        e.preventDefault();
        $btnName = $(this).text();
        ...
        var formData = new FormData($('#addProjectFolder')[0]);
        const attributes = {
            hasButton: true,
            btnSelector: '.addProjectFolderBtn',
            btnText: $btnName,
            handleSuccess: function() {
                toastr.success(datas.msg)
                $("#create_new_project_folder").modal('hide');
                $('td[name="name"]').val('');
    ...
    function loadProjectFolderData(viewfilefolders) {
        let formData = new FormData();
        let project_id = $('.projectgobackbtn').attr('data-project-id');
        formData.append(
            "table_search_input",
            $("input[name=table_search_input]").val() || ""
        );
        if (viewfilefolders == 'viewfilefolders') {
            formData.append(
                "type", "viewfilefolders"
            );
        } else {
            formData.append(
                "type", "projectfolders"
        ...
        const attributes = {
            ...
            handleSuccess: function() {
                ...
                if (viewfilefolders == 'viewfilefolders') {
                    ...
                    $('.view-folders-row').html(datas['project_load_folder']);
                } else {
                    console.log(datas['project_load_folder']);
                    $('.project-folders-row').html(datas['project_load_folder']);
                }
    ...
    let fileTable = $(".project-folders-row");
    let folderId = null;
    let resumable;
    $(document).on('click', '.projectgobackbtn', function(e) {
        $('.create_new_folder_btn').show();
        $('.form_file').addClass('d-none');
        $('.projectgobackbtn').addClass('d-none');
        $('.projectgobackbtn').next('h4').removeClass('w-75').addClass('w-100');
        $('.projectgobackbtn').next('h4').text('Upload New File');
        $('.custom_width-modal-folder').removeClass('d-none');
        $(".uploadProjectDcoumentsModal").removeClass("gradiant-button");
        $('.uploadProjectDcoumentsModal').attr('disabled', true);
        loadProjectFolderData();
    });
    ...
    $(document).on("click", ".projectfolders", function(e) {
    ...
    $(document).on("click", ".viewfilefolders", function(e) {
        folderId = $(this).data("folder-id");
        $(this).parent('.folder_box').addClass('loader');
        loadViewFiles(folderId);
        $('.custom_width-modal-folder').addClass('d-none');

    });

    var fileIdArray = []
    $(document).on("click", ".file_remove_btn", function(e) {
        // fileIdArray
        // $('.data_file_id').val($(this).data('id'));
    ...
    $(document).on('click', '.gobackbtnshowUploadedModal', function(e) {
        loadProjectFolderData('viewfilefolders');
        $('.gobackbtnshowUploadedModal').addClass('d-none');
        $('.gobackbtnshowUploadedModal').next('h4').removeClass('w-75').addClass('w-100');
        $('.gobackbtnshowUploadedModal').next('h4').text('Upload New File');
        $('.custom_width-modal-folder').removeClass('d-none');

    });

    function loadViewFiles(folderId) {
        ...
        const attributes = {
            hasButton: false,
            handleSuccess: function() {
                $('.view-folders-row').html(datas['files']);
                $('.gobackbtnshowUploadedModal').removeClass('d-none').css('width', '10%');
                $('.gobackbtnshowUploadedModal').next('h4').removeClass('w-100').addClass(
                        'w-75')
                    .text(datas[
                        'foldername']['name']);
    ...
    function loadProjectFiles(folderId) {
        var routeUrl = routeUrlStoreFiles.replace(":id", folderId);
        var getFilesUrl = getFiles.replace(":id", folderId);
        ...
        const attributes = {
            ...
            handleSuccess: function() {
                ...
                $('.project-folders-row').html(datas['files']);

                $('.projectgobackbtn').removeClass('d-none').css('width', '10%');
                $('.projectgobackbtn').next('h4').removeClass('w-100').addClass('w-75').text(datas[
                    'foldername']['name']);
    ...
    $('.handleUploadProjectDocumentsFile').on('change', function() {
        ...
        resumable.on("fileProgress", function(file) {
            ...
            $(`#file-${file.uniqueIdentifier} .progress-value`).text(
                `${progressValue}%`);
        });
    });


    // $(document).on("change", ".handleUploadProjectDocumentsFile", function(event) {
    //     console.log(242342);
    ...
    $(document).on("change", ".uploadDocumentsCheckbox", function(e) {
        if ($('.uploadDocumentsCheckbox:checked').length > 0) {
            $(".uploadProjectDcoumentsModal").addClass("gradiant-button");
            $('.uploadProjectDcoumentsModal').removeAttr("disabled")
        } else {
            $(".uploadProjectDcoumentsModal").removeClass("gradiant-button");
            $('.uploadProjectDcoumentsModal').attr('disabled', true);
    ...
    $(document).on("click", "#projectFileFolderModalClose", function(e) {
    ...
    $(document).on("click", "#viewFileFolderModalClose", function(e) {
        $('.gobackbtnshowUploadedModal').addClass('d-none');
        $('.gobackbtnshowUploadedModal').next('h4').removeClass('w-75').addClass('w-100');
        $('.gobackbtnshowUploadedModal').next('h4').text('Upload New File');
        $('.custom_width-modal-folder').removeClass('d-none');
    ...
    $(document).on("click", ".data_file_id", function(e) {
        let formData = new FormData();
        fileIdArray.forEach(function(val) {
            // Append each fileId to the formData with a key like 'fileId[]'
            formData.append('fileId[]', val);
        });
